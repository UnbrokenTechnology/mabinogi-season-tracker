...
- **Pages**: Dashboard (chips, phase banner, counter dials, 4 checklists, heat strip, DeadlineRail, KeyLedgerWidget), Planner (strategy/spec chooser — tailors tasks & milestones), Progress, GoldGuide (dims streams the user's build can't run), MaistirGuide (`/maistir` — path-to-rank steps, rank tiers, KR treadmill rules, currency cards + spend-priority tiers + permanent-vs-temporary card; data in `maistirGuide.ts`, lines render "Lead — detail" with bold lead), Market (`/market` — cauldron craft-profit calculator: enter each material price once + per-recipe sell prices, profit = sell×(1−fee)−materials via `src/lib/market.ts` (Vitest-covered); table rows stay in unlock order so inputs don't jump while typing — best row is gold-highlighted, winners surface in the "Best right now" bar; share links via `src/lib/share.ts` encode the sheet positionally into `#/market?p=…`, imported by a route-query watcher whose dialog needs `noRouteDismiss: true` because the query is stripped while it's open — Quasar dialogs dismiss on route change; Farm Planner card ranks per-material best use (`materialBestUse`: max of raw net-sale vs best recipe's net minus other ingredients ÷ qty consumed) into gold per plot-hour with editable plot counts in the market store; price clearing lives ONLY in Settings' danger zone with confirm + undo — `clearPrices` snapshots to `clearedBackup` and the Market page shows a Restore button while prices are empty; a user lost their sheet to the old un-confirmed Clear-all that sat next to Share link — don't put destructive buttons beside common ones; Raw Crops Calculations card shows sell-raw-only income in fixed side-by-side 1h/8h columns; True Ratio Planner card runs `src/lib/ratioPlan.ts` — a from-scratch LP (grid over field splits × primal simplex, per-cauldron 60 min/hr caps, surplus sold raw, 5 Vitest cases) giving the honest self-grown gold/hr, deliberately lower than Max Profit Planner's marginal sums which double-count shared recipe margins), Almanac (per-week archive derived from counters/tasks/ledger — nothing extra tracked), Ledger, Settings (JSON export/import, reset).
